...
public:
    ...
    // Declaració de les funcions creades.
    void drawRectangle(unsigned int x, unsigned int y, unsigned int w, unsigned int h, Color color, bool fill);
	void drawCircle(unsigned int x, unsigned int y, unsigned int r, Color color, bool fill);
	void drawLine(unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2, Color color);
    void workWithBasicFigures(Image& framebuffer);

	void drawGradientBtoR();
    void sinusoidalGradient();
    void radialGradient();
    void bandwBoard();

    void originalPhoto(Image& picture);
    void grayscalePhoto(Image& picture);
    void invertedPhoto(Image& picture);

    void rotateImage( Image& picture, float angle);
    void scaleImage(Image& picture, float scale);
};
